...
在模型取得了 93% 的高准确率之后，本节将通过混淆矩阵与具体的错误样本剖析，进一步探究 Transformer 模型在当前任务中的局限性，从而完成闭环的批判性评估。

### 5.1 全局定量评估 (混淆矩阵)
我们在 25,000 条测试集数据上对模型进行了无监督的全局推理，并绘制了混淆矩阵（Confusion Matrix）以观察模型在不同类别上的表现差异。

> **[📝 提示：在此处插入你刚刚保存的混淆矩阵图片]**
> *图 3：测试集评估混淆矩阵*

由混淆矩阵可知，在总计预测错误的 **[填写代码输出的总错误数，例如：1742]** 条样本中：
* **假阳性 (False Positives)**，即实际为负面评论但被模型误判为正面的数量为 **[填写代码输出的FP数量]**。
* **假阴性 (False Negatives)**，即实际为正面评论但被模型误判为负面的数量为 **[填写代码输出的FN数量]**。

两类错误的数量相对均衡，这不仅与前文 F1-Score (0.93) 的高得分相呼应，也再次印证了模型并未产生明显的偏见（Bias），没有倾向于无脑盲猜某一类别。

### 5.2 典型错误案例分析 (Qualitative Analysis)
为了探究为什么具有深度语义理解能力的 DistilBERT 依然会犯错，本项目从预测失败的样本中提取了典型案例进行人工定性分析。

**【案例剖析：难以识别的反讽与复杂语境】**
*   **原始文本**：*"[在这里复制粘贴上一段代码打印出来的一个比较有意思的错误文本，最好是带一点阴阳怪气或者欲扬先抑的段落]"*
*   **实际标签**：[填写：负面 / 正面]
*   **模型预测**：[填写：正面 / 负面]
*   **人工分析**：分析该样本发现，作者在评论中使用了大量的**[填写你的分析，例如：反讽手法 / 欲扬先抑的修辞 / 引用了电影里反派的糟糕台词]**。模型虽然捕捉到了诸如 "[这里挑几个文本里的正面词，比如 'good', 'masterpiece']" 等局部的情感极性词汇，但由于缺乏对人类复杂讽刺逻辑的常识推理能力，导致其被表象词汇误导。此外，在数据预处理阶段，我们将最大截断长度设置为了 512，如果一篇长评的“真正总结性结论”出现在文章最末尾（大于 512 词的位置），关键信息的丢失也会直接导致模型误判。

上述分析表明，尽管基于 Transformer 的预训练模型已能解决大部分浅层情感分类问题，但在处理**长文本逻辑反转、重度反讽修辞以及特定领域常识依赖**的任务时，仍存在显著的优化空间。
